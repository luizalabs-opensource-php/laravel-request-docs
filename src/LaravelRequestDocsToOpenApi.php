...
class LaravelRequestDocsToOpenApi
{
        ...
        foreach ($docs as $doc) {
            ...
            foreach ($doc['rules'] as $attribute => $rules) {
                foreach ($rules as $rule) {
                    if ($isPost || $isPut || $isDelete) {
                        $requestHasFile = $this->attributeIsFile(runTheLast($rule));
            ...
            foreach ($doc['rules'] as $attribute => $rules) {
                foreach ($rules as $rule) {
                    ...
                    if ($isPost || $isPut || $isDelete) {
                        $this->openApi['paths'][$doc['uri']][$httpMethod]['requestBody']['content'][$contentType]['schema']['properties'][$attribute] = $this->makeRequestBodyContentPropertyItem(runTheLast($rule));
                    }
                }
            }
        }
    ...
    protected function makeRequestBodyContentPropertyItem(string $rule): array
    {
        ...
        return [
            ...
            'format' => $this->attributeIsFile(runTheLast($rule)) ? 'binary' : $type,
        ];
    }
    ...
    {
        if (str_contains($rule, 'string') || $this->attributeIsFile(runTheLast($rule))) {
            return 'string';
        }
